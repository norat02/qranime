...
$textConfig = [
    'fontPath' => __DIR__ . '/../../font/sg85_chi_son/SG85-CHISON1.ttf',
    'fontSizeMax' => 26, // Reduced to fit small box (64px height)
    'colorHex' => '#e009e8ff',
    'strokeHex' => '#ffffff',
    'shadowAlpha' => 115,
    'box' => [
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if (file_exists($textConfig['fontPath'])) {
            ...
            // Colors for Default Style (Black, No Stroke, No Shadow)
            $colBlack = imagecolorallocate($finalImage, 0, 0, 0);
            $colTransparent = imagecolorallocatealpha($finalImage, 0, 0, 0, 127);
            ...
            $drawer->drawFitText(
                ...
                $maxLineHeight,
                $colBlack,
                $colTransparent, // No Stroke
                $colTransparent, // No Shadow
...
class TextDrawer
{
    ...
    public function drawFitText($text, $refX, $refY, $maxWidth, $maxHeight, $color, $strokeColor, $shadowColor, $maxFontSize, $align = 'center')
    {
        ...
        $drawX = $refX;
        if ($align === 'center') {
            $drawX = $refX - ($textW / 2); // refX is center X
        }
        // if align left, refX is left X, so drawX = refX (minimal adjustment needed for bbox offset usually ignored for simple left align or can be fine tuned)

        $drawY = $refY + ($textH / 3); // Approximate vertical centering correction
